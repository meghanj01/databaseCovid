...
def post_appointments():
    ...
    if request.method == "POST" and request.is_json:
        ...
        req["patient_id"] = validate_patient_profile(
            db, req["patient_name"], req["insurance"]
        )
        ...
        return jsonify(
            {
                "message": f"Your appointment is booked. Appointment number: {str(appointment_id)}. Please check the details by clicking on info.",
                "status": 200,
            }
        )
...
def cancel_appointment(id):
    ...
    db = database.get_db_connection()
    validate_appointment_id(db, id)
    cancel_appointment_by_id(db, id)
    database.close_db(db)

    return jsonify(
        {"message": "Your appointment is cancelled successfully", "status": 200}
    )
